      ...
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Valor del API vs override manual. Guardar un override marca el snapshot del mes como pendiente de recalcular.
          </p>
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Fuente vigente</TableHead>
          ...
          <TableBody>
            {(countries as string[]).flatMap((currency) =>
              months.map((m) => <Row key={`${currency}-${m}`} currency={currency} month={m} api={apiFor(currency, m)} override={overrideFor(currency, m)} onSave={upsert.mutate} onDelete={del.mutate} disabled={!isAdmin} />)
            )}
          </TableBody>
        </Table>
      </div>
...
function Row({ currency, month, api, override, onSave, onDelete, disabled }: any) {
  ...
  return (
    <TableRow>
      ...
      <TableCell className="text-xs">{api != null ? api.toFixed(4) : "—"}</TableCell>
      <TableCell>
        <Input type="number" step="0.0001" value={val} onChange={(e) => setVal(e.target.value)} className="w-28 h-8" disabled={disabled} placeholder="—" />
      </TableCell>
      <TableCell>
      ...
      <TableCell className="text-right">
        <div className="flex gap-1 justify-end">
          <Button size="sm" variant="outline" disabled={disabled || !val} onClick={() => onSave({ currency, month, rate: Number(val), prefer })}>
            Guardar
          </Button>
